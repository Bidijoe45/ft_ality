...
type trie = {
  ...
  accepting_states : (state * Grammar.combo_name list) list
}
...
let find_key_mapping (key : Grammar.key) (mappings : Grammar.key_mapping list) = List.find_opt (fun (k, _) -> k = key) mappings
...
let find_accepting_state (state : state) (accepting_states : (state * Grammar.combo_name list) list) = List.find_opt (fun (s, _) -> s = state) accepting_states
